...
namespace Mastic
{
    public class Glacier : NetworkBehaviour, IMovementAbility
    {
        [SerializeField] private EasyBinding ability2 = default;
        [SerializeField] private GameObject glacierPrefab = default;
        ...
        private GameObject glacierVisual;
        private BuildableEntity glacierEntity;
        ...
        private void Awake()
        {
            ...
            glacierVisual = Instantiate(glacierPrefab, glacierPrefab.transform.position, glacierPrefab.transform.rotation);
            cam = GameObject.FindWithTag("MainCamera").transform;
            glacierVisual.SetActive(false);
        ...
        {
            base.OnStartServer();
            glacierEntity = glacierVisual.GetComponent<BuildableEntity>();
            glacierEntity.SetCollision(false);
            glacierEntity.Initialize(FindAnyObjectByType<LagCompensation>());
        }
        ...
        {
            Destroy(glacierVisual);
            EventManager<int>.RemoveListener(Occasion.DoUnlocalMovementAbilities, DoUnlocalTick);
        }

        private void SetScaleWithTickProgress(int progress)
        {
            float percentage = Mathf.Clamp(progress, 0f, tickDuration) / tickDuration;
            glacierVisual.transform.localScale = Vector3.one * percentage;
        }
        ...
        private void SetAs(int inputTick)
        {
            bool active = IsAbilityActive(inputTick);
            if (isServer)
                glacierEntity.SetCollision(active);

            glacierVisual.SetActive(active);
            if (active)
                SetScaleWithTickProgress(inputTick - startingTick);
        }
